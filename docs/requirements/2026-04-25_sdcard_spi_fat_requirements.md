...
- 初期実装は SDHC、FAT32、512 バイト sector、8.3 filename、単一 volume、root directory に限定する。
...
Root directory の初期構成例は次の通り。

| ファイル | 用途 |
| --- | --- |
| `SDFS.BIN` | 第2段 SD/FAT driver、DIR/LF、AUTOEXEC.S 処理を持つ M6800 DOS 相当 |
| `AUTOEXEC.S` | `SDFS.BIN` 起動後に任意で LOAD/RUN する S-Record |
...
6. `AUTOEXEC.S` の LOAD が成功し、実行アドレスが決まる場合だけ RUN する。

Reserved sector bootstrap 案の起動順序は次の通り。

1. ROM が SDHC 初期化、MBR/BPB 最小読取、reserved sector read、signature 検査を行う。
2. ROM が第1段 bootstrap を `$C000` 付近へ読み込み、実行する。
3. 第1段 bootstrap が root directory から `SDFS.BIN` を探す。
4. 第1段 bootstrap が `SDFS.BIN` を第1段自身と衝突しない RAM 領域へ読み込む。
...
`AUTOEXEC.S` の自動実行アドレスは、初期案では S9 終端レコードの 16bit 実行アドレスのみを RUN 対象にする。S8/S3 系の 24bit アドレス、実行アドレスなし、固定エントリ規約は初期対象外とする。Intel HEX の自動実行は初期対象外とし、手動 LOAD 用として扱う。

`AUTOEXEC.S` が存在しない場合は、エラー扱いにせず `SDFS.BIN` の対話モードへ進む。`AUTOEXEC.S` の読み取り失敗、S-Record 解析失敗、実行アドレス未確定の場合は、短いエラー表示後に対話モードへ戻る。

暴走する `AUTOEXEC.S` から復旧できるように、起動時のキー入力で `AUTOEXEC.S` 自動実行を skip できる手段を初期実装候補にする。skip 手段の具体的なキーと受付タイミングは実装時に決める。

`CONFIG.SYS` は現時点では初期対象外とする。既定 directory、RTC 種別、画面/キーボード種別、自動実行ファイル名、メモリ配置設定などが必要になった段階で将来候補として検討する。
...
- 第1段 bootstrap は `$C000-$DFFF` 上で動作し、FAT32 read-only の最小処理で root directory の第2段本体を読む。
- 第2段本体は FAT 上の通常ファイル `SDFS.BIN` に置く。
- `$C000-$DFFF` は第1段 bootstrap、512 バイト sector buffer、SD/FAT driver、DIR/LF 処理、第2段 loader に使う。
- 第1段 bootstrap と sector buffer は `$C000` 付近の小領域を使い、`SDFS.BIN` は第1段自身を上書きしない後続領域へ読み込む。候補配置は第1段を `$C000-$C7FF`、sector buffer を `$C800-$C9FF`、`SDFS.BIN` を `$CA00` 以降とするが、実際の境界はサイズ見積もり後に決める。
- signature 不一致、reserved sector 不足、read error、`SDFS.BIN` 未検出、8.3 名不一致、サイズ不正、ヘッダ不正、FAT chain read error 時は、従来のシリアル `L` へ fallback する。

ROM 直 FAT fallback を持たせる場合、bootstrap 案は ROM 削減案ではなく冗長起動手段になる。そのため ROM 削減を目的とする bootstrap 案では、fallback はシリアル `L` のみに限定する。

SD 初期設定の面倒さは、PC 側スクリプトで吸収する。専用 SD イメージまたは専用作成ツールで FAT32 format、reserved sector への第1段書き込み、root directory への `SDFS.BIN` 配置、検証用 signature 確認までをまとめて行う。
...
5. root directory の entry を読み、8.3 ファイル名とサイズを `DIR` で表示する。
...
9. Root directory の `SDFS.BIN` を第2段として読み込めることを確認する。
10. Reserved sector bootstrap 案では、第1段 bootstrap と `SDFS.BIN` のロード先が自己上書きしないことを確認する。
11. `SDFS.BIN` 未検出、サイズ不正、ヘッダ不正、FAT chain read error でシリアル `L` fallback へ落ちることを確認する。
12. `SDFS.BIN` 起動後、root directory の `AUTOEXEC.S` が存在する場合だけ LOAD/RUN 候補になることを確認する。
13. `AUTOEXEC.S` が存在しない場合、エラー扱いにせず対話モードへ進むことを確認する。
14. `AUTOEXEC.S` の読取失敗、S-Record 解析失敗、S9 実行アドレス未確定時はエラー表示後に対話モードへ戻ることを確認する。
15. 起動時キー入力で `AUTOEXEC.S` 自動実行を skip できることを確認する。
16. `BPB_RsvdSecCnt` 不足、候補 sector 範囲不足、signature mismatch、read error でシリアル `L` fallback へ落ちることを確認する。
17. Windows で通常ファイルコピー、取り外し、再マウントを行い、raw sector の before/after 比較で第1段が変化しないことを確認する。
18. format、`chkdsk`、ディスク管理ツール操作は第1段を破壊し得る操作として扱い、専用作成ツールで再作成できることを確認する。
19. SAVE と外付けマイコン方式は将来フェーズで検討する。

## 実装時の受け入れ条件

- PIA 直結 PoC では、SD カード初期化と固定 LBA sector read が成功すること。
- FAT32 BPB を読み、FAT32 判定と基本パラメータ取得ができること。
- Root directory の `SDFS.BIN` を第2段本体として扱うこと。
- ROM 直 FAT 案と reserved sector bootstrap 案の起動責務が分離されていること。
...
- `AUTOEXEC.S` は root directory に存在する場合だけ `SDFS.BIN` 起動後の LOAD/RUN 候補になること。
- `AUTOEXEC.S` が存在しない場合、エラー扱いにせず対話モードへ進むこと。
- `AUTOEXEC.S` の自動 RUN は S9 終端レコードの 16bit 実行アドレスに限定すること。
- `AUTOEXEC.S` の読取失敗、S-Record 解析失敗、S9 実行アドレス未確定時はエラー表示後に対話モードへ戻ること。
- 起動時キー入力で `AUTOEXEC.S` 自動実行を skip できる候補を仕様に含めること。
- `CONFIG.SYS` は初期対象外であり、将来候補として扱われること。
- Reserved sector bootstrap 案では、signature 付き第1段を `$C000` へ読み込み、signature 確認後に実行できること。
- Reserved sector bootstrap 案では、第1段 bootstrap と `SDFS.BIN` のロード先が自己上書きしないこと。
- `SDFS.BIN` 未検出、サイズ不正、ヘッダ不正、FAT chain read error 時はシリアル `L` fallback へ落ちること。
...
- 将来 `/MC6800/` 固定 directory をサポートするか。
- `AUTOEXEC.S` の自動実行アドレスを S-Record 終端レコードで扱うか、固定エントリ規約で扱うか。
- `CONFIG.SYS` を将来導入する場合、どの設定項目を対象にするか。
