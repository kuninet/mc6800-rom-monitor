...
履歴確認では、分岐元の `main`、つまり merge-base `392336e` の時点で `CMD_RESUME` はすでに `BRK_SAVE_*` から SWI フレームへ書き戻す正しい向きだった。PoC 実装コミット `0903a85` で一度 SWI フレームから `BRK_SAVE_*` へ読み戻す逆向きに変わり、その後の `35a49e2` で元の正しい向きへ戻されている。

したがって、#47 で扱うべき内容は「main に残っていた `CMD_RESUME` バグの救出」ではなく、「PoC で一度壊れた箇所が現行 `main` では正しいことを確認し、回帰テストで固定する」ことである。
